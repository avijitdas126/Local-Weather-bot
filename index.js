...
bot.hears("/now", async (ctx) => {
  ...
  let data = await fetch(
    `https://api.openweathermap.org/data/2.5/weather?q=${city}&appid=${process.env.OpenWeather_Token}&units=metric`
  );
...
bot.on(message("text"), async (ctx) => {
  ...
  if (text.includes("/setlocal")) {
    ...
    if (data.cod == 200) {
      ...
      ctx.reply(
        `${data.message}. Set Your local city correctly using /setlocal`
      );
    }
  }
});
